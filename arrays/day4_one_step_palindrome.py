# Palindrome checker with one deletion step

def is_palindrome_range(left, right, input_array):
    while left < right:
...
def is_palindrome(input_array):
    ...
    while left < right:
        if input_array[left] != input_array[right]:
            return is_palindrome_range(left + 1, right, input_array) or is_palindrome_range(left, right - 1, input_array)
        left += 1 
        right -= 1 
    return True
